...
int main(int nargs, char **args) {
    ...
    if (my_rank == 0) {
        ...
        print_2d_array(input, N, M);
        printf("\n");
        printf("KERNEL1 2D ARRAY:\n");
        print_2d_array(kernel1, K1, K1);
        printf("\n");
        printf("KERNEL2 2D ARRAY:\n");
        print_2d_array(kernel2, K2, K2);
        printf("\n");
    ...
    int *scatter_displacements = malloc(number_of_processes * sizeof *scatter_displacements);  // Should hold where every process starts reading from input
    int *gather_displacements = malloc(number_of_processes * sizeof *gather_displacements);    // Should hold where every process should gather the data from
    int rows = (N / number_of_processes);                                                           // Number of rows per process (some will get additional rows). It should be chunks of K1 rows.
    int remainder = N % number_of_processes;                                                        // Should hold the remainder when N is no t divisible with number of processes
    ...
    // Broadcast scatter/gather values to all other processes
    MPI_Bcast(n_rows, number_of_processes, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(sendcounts, number_of_processes, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(scatter_displacements, number_of_processes, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(gather_displacements, number_of_processes, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    // Execute double layer convolution in parallel with scatter inside the function
    ...
    if (my_rank==0) {
        ...
        printf("PARALLEL 2D ARRAY OUTPUT:\n");
        print_2d_array(output, N-K1-K2+2, M-K1-K2+2);
